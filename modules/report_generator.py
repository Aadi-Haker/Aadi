"""
Aadi — Report Generator Module
Author: Aaditya Kumar Pandey | Instagram: @aadi_97621
"""
...
<title>Aadi — Security Report</title>
<style>
...
<header>
  <div>
    <div class="logo">👻 Aadi<span>Advanced Android Pentesting Framework</span></div>
  </div>
  <div class="meta">
    <div class="target">{target}</div>
    <div>Generated: {timestamp}</div>
    <div>Report ID: {report_id}</div>
    <div>Author: Aaditya Kumar Pandey | @aadi_97621</div>
  </div>
</header>
<main>
...
<footer>
  <p>Aadi &copy; 2026 | Author: <a href="https://www.instagram.com/aadi_97621">Aaditya Kumar Pandey</a> | For authorized use only</p>
</footer>
</body>
</html>
"""
...
def generate_html_report(data: dict, output: str = "aadi_report.html") -> str:
    """Generate a styled HTML report."""
    findings = data.get("findings", [])
    target   = data.get("target", "Unknown Target")
    ts       = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    report_id = f"AADI-{int(time.time())}"
    rems = _remediations()
...
def generate_json_report(data: dict, output: str = "aadi_report.json") -> str:
    """Generate a structured JSON report."""
    report = {
        "tool": "Aadi",
        "author": "Aaditya Kumar Pandey | @aadi_97621",
...
def print_summary_table(data: dict):
    ...
    t = Table(title="[bold magenta]📋 Aadi — Findings Summary[/]",
